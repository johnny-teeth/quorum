...
void read_pages(int page) {
    ...
    if (strncmp(req, (char *)"UNABLE", 5) == 0) {
        skip = 1;
    }

    fprintf(stdout, "%s%s%s%s\n",welcome[0], read_action_hdr, welcome[0], req);
    printf("Performed in: %lu clockticks\n", timer);
    if (skip) {
        run_ui(loc_sock);
        return;
    }
...
void display_replies(int artnum) {
    ...
    msg = malloc(snprintf(NULL, 0, "READ:%d.1", artnum));
    sprintf(msg, "READ:%d.1", artnum);
    timer = clock();
	write(loc_sock, msg, strlen(msg));
	free(msg);
    
	msg = malloc(MAX_PAGE_MSG);
	rdsz = read(loc_sock, msg, MAX_MSG);
    timer = clock() - timer;
	msg[rdsz] = '\0';
    
    if (strncmp(msg, "UNABLE", 5) == 0) {
        skip = 1;
    }
...
void reply_menu(int artnum) {
    ...
    msg = malloc(MAX_MSG);
	if((rdsz = read(loc_sock, msg, MAX_MSG)) <= 0) {
...
void post_menu() {
    ...
    timer = clock() - timer;
	resp[rdsz] = '\0';
    if (strncmp(resp, "UNABLE", 5) == 0) {
        fprintf(stdout, "%s%s\nPerformed in: %lu clockticks\n%s", welcome[0], resp, timer, welcome[0]);
        run_ui(loc_sock);
        return;
    }
    
	fprintf(stdout, "%s%s%s", welcome[0], post_action_hdr, welcome[0]);
	...
	cntn[rdsz] = '\0';
    time2 = clock();
	if(write(loc_sock, cntn, strlen(cntn)) <= 0) {
